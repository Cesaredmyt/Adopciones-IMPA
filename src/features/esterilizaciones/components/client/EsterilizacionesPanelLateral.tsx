  ...
  return (
    <aside className="space-y-4">
      <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <h3 className="text-sm font-bold text-[#8B4513] mb-3 uppercase tracking-wider">
          Resumen rápido
        </h3>
        <ul className="space-y-2 text-sm">
          ...
          <li className="flex justify-between">
            ...
            <span className="font-bold text-amber-700">{aprobadas}</span>
          </li>
          <li className="flex justify-between">
        ...
        {proximas.length === 0 ? (
          ...
          <ul className="space-y-3">
            {proximas.map((p) => (
              <li
                key={p.id}
                className="flex items-center gap-3 text-sm border-l-4 border-[#BC5F36] pl-3"
              >
                {p.mascota_imagen ? (
                  <img
                  ...
                  <div className="w-9 h-9 rounded-md bg-orange-100" />
                )}
