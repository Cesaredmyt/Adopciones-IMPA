...
export default function AppointmentItem({ cita, onEliminar }: { cita: Cita; onEliminar: (id: string) => void }) {
  return (
    <li className="rounded-2xl border border-[#dce5dc] bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#ecfdec] text-[#17cf17]">
            <PawPrint size={18} />
          </div>
          <div>
            <div className="flex flex-wrap items-center gap-x-2 text-[15px] text-[#6b4d3e]">
              <span className="font-extrabold text-[#111811]">{cita.mascota}</span>
              <span>•</span>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-xl border border-[#dce5dc] bg-white px-3 py-2 text-xs font-bold text-[#8a5d49] hover:bg-[#ecfdec]"
          >
